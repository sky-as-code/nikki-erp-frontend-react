...
import { Center, Group, Paper, Stack, Switch, Text, Title } from '@mantine/core';
	...
	return (
		<Stack gap='md'>
			...
			{state.requestErrors.length > 0 ? (
				<Stack gap={2} {...tid('requestErrors')}>
					{state.requestErrors.map((item, i) => (
						<Text key={i} size='sm' c='red'>{t(item.key, item.vars) || item.message}</Text>
					))}
				</Stack>
			) : null}
...
function SortableCell({ id, text, placeholder }: SortableCellProps): React.ReactNode {
	...
	return (
		<Paper
			...
			px='xs'
			h={ROW_HEIGHT}
			className={clsx('flex items-center select-none', isDragging && 'opacity-60')}
			style={{ transform: CSS.Transform.toString(transform), transition }}
		>
			<Group gap='xs' wrap='nowrap' className='min-w-0 w-full'>
				<span className='cursor-grab flex items-center' {...attributes} {...listeners}>
					<IconGripVertical size={16} />
				</span>
				<Text size='sm' truncate c={text ? undefined : 'dimmed'}>{text ?? placeholder}</Text>
			</Group>
		</Paper>
	);
}
