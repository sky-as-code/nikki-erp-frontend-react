...
export const KioskSettingCreatePage: React.FC = () => {
	...
	const schema = kioskSettingCreateSchema as ModelSchema;
	const { isSubmitting, handleCancel, handleSubmit } = useKioskSettingCreate();

	React.useEffect(() => {
		document.title = translate('nikki.vendingMachine.kioskSettings.title_create');
	}, [translate]);

	const breadcrumbs = useMemo(() => [
		{ title: translate('nikki.vendingMachine.title'), href: '../overview' },
		{ title: translate('nikki.vendingMachine.kioskSettings.title'), href: '../kiosk-settings' },
		{ title: translate('nikki.vendingMachine.kioskSettings.title_create'), href: '#' },
	], [translate]);
