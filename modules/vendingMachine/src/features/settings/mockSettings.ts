...
export const mockSettings = {
	...
	async getSetting(id: string): Promise<Setting | undefined> {
		await delay(300);
		return mockSettingsData.find((s) => s.id === id);
	},
	...
	async createSetting(setting: Omit<Setting, 'id' | 'createdAt' | 'etag'>): Promise<Setting> {
		...
		const newSetting: Setting = {
			...setting,
			id: String(mockSettingsData.length + 1),
			createdAt: new Date().toISOString(),
			etag: `etag-set-${Date.now()}`,
		};
		mockSettingsData.push(newSetting);
		return newSetting;
	},
	...
	async updateSetting(
		...
		await delay(500);
		const index = mockSettingsData.findIndex((s) => s.id === id);
		if (index === -1) {
			throw new Error('Setting not found');
		}
		const updatedSetting: Setting = {
			...mockSettingsData[index],
			...updates,
			etag: `etag-set-${Date.now()}`,
		};
		mockSettingsData[index] = updatedSetting;
	...
	async deleteSetting(id: string): Promise<void> {
		...
		mockSettingsData.splice(index, 1);
	},
};
