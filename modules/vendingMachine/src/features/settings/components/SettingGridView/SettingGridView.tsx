...
export const SettingGridView: React.FC<SettingGridViewProps> = ({
	...
	if (settings.length === 0) {
		return <Text c='dimmed'>{translate('nikki.vendingMachine.settings.messages.no_settings')}</Text>;
	}
				...
				>
					<Stack gap='sm'>
						...
						<Group gap='xs' wrap='nowrap'>
							<Badge variant='light' color='blue' size='sm'>{setting.category}</Badge>
							{getStatusBadge(setting.status)}
						</Group>

						<Text size='xs' c='dimmed' fw={500}>
							{translate('nikki.vendingMachine.settings.fields.value')}: <Text span c='dark'>{setting.value}</Text>
						</Text>

						<Text size='xs' c='dimmed'>
							{translate('nikki.vendingMachine.settings.fields.createdAt')}: {new Date(setting.createdAt).toLocaleDateString()}
						</Text>
					</Stack>
				</Card>
			))}
