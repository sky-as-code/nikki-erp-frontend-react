...
export interface SlideshowTableProps extends AutoTableProps {
...
const NameColumn: React.FC<{ row: Record<string, unknown> }> = ({ row }) => {
	...
	const slideshowId = row.id as string;
...
function renderActionsColumn(
	row: Record<string, unknown>,
	onView?: (slideshowId: string) => void,
	onEdit?: (slideshowId: string) => void,
	onDelete?: (slideshowId: string) => void,
	translate?: (key: string) => string,
) {
			...
			<Group gap='xs' justify='flex-end' onClick={(e) => e.stopPropagation()}>
				{onView && (
					<Tooltip label={translate('nikki.general.actions.view')}>
						<ActionIcon variant='subtle' color='blue' onClick={() => onView(slideshowId)}>
							<IconEye size={16} />
						</ActionIcon>
					</Tooltip>
				)}
				{onEdit && (
					<Tooltip label={translate('nikki.general.actions.edit')}>
						<ActionIcon variant='subtle' color='gray' onClick={() => onEdit(slideshowId)}>
							<IconEdit size={16} />
						</ActionIcon>
					</Tooltip>
				)}
				{onDelete && (
					<Tooltip label={translate('nikki.general.actions.delete')}>
						<ActionIcon variant='subtle' color='red' onClick={() => onDelete(slideshowId)}>
							<IconTrash size={16} />
						</ActionIcon>
					</Tooltip>
				)}
...
export const SlideshowTable: React.FC<SlideshowTableProps> = ({
	columns,
	data,
	schema,
	isLoading,
