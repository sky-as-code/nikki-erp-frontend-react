...
const NameColumn: React.FC<{ row: Record<string, unknown> }> = ({ row }) => {
	...
	const settingId = row.id as string;
	const name = String(row.name || '');

	const handleClick = (e: React.MouseEvent) => {
		e.stopPropagation();
		if (settingId) {
			navigate(`../kiosk-settings/${settingId}`);
		}
	};
...
function renderValueColumn(row: Record<string, unknown>) {
	return <code>{String(row.value || '')}</code>;
}
			...
			<Group gap='xs' justify='flex-end' onClick={(e) => e.stopPropagation()}>
				{onView && (
					<Tooltip label={translate('nikki.general.actions.view')}>
						<ActionIcon variant='subtle' color='blue' onClick={() => onView(settingId)}>
							<IconEye size={16} />
						</ActionIcon>
					</Tooltip>
				)}
				{onEdit && (
					<Tooltip label={translate('nikki.general.actions.edit')}>
						<ActionIcon variant='subtle' color='gray' onClick={() => onEdit(settingId)}>
							<IconEdit size={16} />
						</ActionIcon>
					</Tooltip>
				)}
				{onDelete && (
					<Tooltip label={translate('nikki.general.actions.delete')}>
						<ActionIcon variant='subtle' color='red' onClick={() => onDelete(settingId)}>
							<IconTrash size={16} />
						</ActionIcon>
					</Tooltip>
				)}
