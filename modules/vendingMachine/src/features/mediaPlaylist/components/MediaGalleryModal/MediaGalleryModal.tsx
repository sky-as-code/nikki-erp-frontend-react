...
export const MediaGalleryModal: React.FC<MediaGalleryModalProps> = ({
	...
	return (
		<Modal
			...
			title={translate('nikki.vendingMachine.mediaPlaylist.media.gallery.title')}
			size='xl'
		...
		>
			<Tabs value={activeTab} onChange={(value) => setActiveTab(value as 'gallery' | 'upload')}>
				<Tabs.List>
					<Tabs.Tab value='gallery' leftSection={<IconGrid3x3 size={16} />}>
						{translate('nikki.vendingMachine.mediaPlaylist.media.gallery.tab')}
					</Tabs.Tab>
					<Tabs.Tab value='upload' leftSection={<IconUpload size={16} />}>
						{translate('nikki.vendingMachine.mediaPlaylist.media.upload.tab')}
					</Tabs.Tab>
				</Tabs.List>
					...
					<Group align='flex-start' gap='md'>
						...
						>
							<Text size='sm' fw={600} mb='xs' px='xs'>
								{translate('nikki.vendingMachine.mediaPlaylist.media.gallery.folders')}
							</Text>
							<ScrollArea h={400}>
								<Stack gap={2}>
									<Box
									...
									>
										...
										<Text size='sm' fw={!selectedFolderId ? 600 : 400}>
											{translate('nikki.vendingMachine.mediaPlaylist.media.gallery.all')}
										</Text>
									</Box>
									{folders.map((folder) => (
										<FolderTreeItem
						...
						<Box style={{ flex: 1 }}>
							<Group justify='space-between' mb='md'>
								<Text size='sm' c='dimmed'>
									{filteredMedia.length} {translate('nikki.vendingMachine.mediaPlaylist.media.gallery.items')}
								</Text>
								<Group gap='xs'>
									<Tooltip label={translate('nikki.vendingMachine.mediaPlaylist.media.gallery.gridView')}>
										<ActionIcon
									...
									<Tooltip label={translate('nikki.vendingMachine.mediaPlaylist.media.gallery.listView')}>
										<ActionIcon
													...
													<Stack gap='xs'>
														<Box style={{ position: 'relative' }}>
															...
															{isAlreadySelected && (
																...
																>
																	{translate('nikki.vendingMachine.mediaPlaylist.media.gallery.selected')}
																</Badge>
															)}
														</Box>
									...
									<Table>
										<Table.Thead>
											<Table.Tr>
												...
												<Table.Th>{translate('nikki.vendingMachine.mediaPlaylist.media.fields.preview')}</Table.Th>
												<Table.Th>{translate('nikki.vendingMachine.mediaPlaylist.media.fields.code')}</Table.Th>
												<Table.Th>{translate('nikki.vendingMachine.mediaPlaylist.media.fields.name')}</Table.Th>
												<Table.Th>{translate('nikki.vendingMachine.mediaPlaylist.media.fields.type')}</Table.Th>
												<Table.Th>{translate('nikki.vendingMachine.mediaPlaylist.media.fields.duration')}</Table.Th>
												<Table.Th>{translate('nikki.vendingMachine.mediaPlaylist.media.fields.size')}</Table.Th>
											</Table.Tr>
										</Table.Thead>
										<Table.Tbody>
											{filteredMedia.map((item) => {
				...
				<Tabs.Panel value='upload' pt='md'>
					<Stack gap='md'>
						<Text size='sm' c='dimmed'>
							{translate('nikki.vendingMachine.mediaPlaylist.media.upload.description')}
						</Text>
						<FileButton
							onChange={(files) => {
						...
						>
							{(props) => (
								<Button {...props} leftSection={<IconUpload size={16} />}>
									{translate('nikki.vendingMachine.mediaPlaylist.media.upload.button')}
								</Button>
							)}
						</FileButton>
					</Stack>
			...
			{selectedMedia.length > 0 && (
				<Stack gap='md' mt='md' pt='md' style={{ borderTop: '1px solid #e9ecef' }}>
					<Text size='sm' fw={500}>
						{translate('nikki.vendingMachine.mediaPlaylist.media.gallery.selectedMedia')} ({selectedMedia.length})
					</Text>
					<ScrollArea h={200}>
						<Stack gap='sm'>
								...
								<Group key={item.id} gap='md' align='flex-start' wrap='nowrap'>
									<MediaPreview
										media={{
											...
											url: item.url,
											thumbnailUrl: item.thumbnailUrl,
											durationSec: mediaDurations[item.id] ?? item.duration,
											order: 0,
										}}
										size='sm'
									/>
									<Stack gap='xs' style={{ flex: 1 }}>
										...
										<Group gap='xs' align='center'>
											<Text size='xs' c='dimmed' style={{ minWidth: 80 }}>
												{translate('nikki.vendingMachine.mediaPlaylist.media.fields.duration')}:
											</Text>
											<NumberInput
												size='xs'
