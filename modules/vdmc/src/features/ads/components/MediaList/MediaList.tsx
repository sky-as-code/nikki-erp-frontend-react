...
export const MediaList: React.FC<MediaListProps> = ({
		...
		return (
			<Stack gap='md'>
				<Group justify='space-between'>
					<Text size='sm' fw={500}>
						{translate('nikki.vendingMachine.ads.media.title')}
					</Text>
					<Button
						size='xs'
						...
						onClick={onAddMedia}
					>
						{translate('nikki.vendingMachine.ads.media.add')}
					</Button>
				</Group>
				<Text size='sm' c='dimmed' ta='center' py='xl'>
					{translate('nikki.vendingMachine.ads.media.empty')}
				</Text>
			</Stack>
		);
	}
	...
	return (
		<Stack gap='md'>
			<Group justify='space-between'>
				<Text size='sm' fw={500}>
					{translate('nikki.vendingMachine.ads.media.title')} ({media.length})
				</Text>
				<Button
					size='xs'
					leftSection={<IconPlus size={16} />}
						...
						.map((item, index) => (
							<Table.Tr key={item.id}>
								...
								<Table.Td>
									...
									>
										{item.type === 'image'
											? translate('nikki.vendingMachine.ads.media.type.image')
											: translate('nikki.vendingMachine.ads.media.type.video')}
									</Badge>
								</Table.Td>
								<Table.Td>
