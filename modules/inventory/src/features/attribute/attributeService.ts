...
export const attributeService = {
	async listAttributes(orgId: string, productId: string): Promise<SearchAttributesResponse> {
		const response = request.unwrapResult(await request.get<SearchAttributesResponse>(
	...
	async getAttribute(
		...
		const response = request.unwrapResult(await request.get<Attribute>(
	...
	async createAttribute(orgId: string, data: CreateAttributeRequest): Promise<CreateAttributeResponse> {
		...
		const response = request.unwrapResult(await request.post<CreateAttributeResponse>(
			`${orgId}/inventory/products/${productId}/attributes`,
			{ json: data }
		));
		return response;
	...
	async updateAttribute(
		...
		const response = request.unwrapResult(await request.put<UpdateAttributeResponse>(
	...
	async deleteAttribute(orgId: string, productId: string, id: string): Promise<DeleteAttributeResponse> {
		const response = request.unwrapResult(await request.del<DeleteAttributeResponse>(
			`${orgId}/inventory/products/${productId}/attributes/${id}`
		));
		return response;
	},
