...
/**
 * Direct REST access for the inventory endpoints `RestApi` does not model — currently the
 * engine's `GET {resource}/:id/effective` read action. Everything that is a plain CRUD call on a
 * registered schema must keep going through `dyn.withSchema(...).restApi` so the schema-etag
