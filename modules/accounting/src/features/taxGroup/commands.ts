...
/**
 * Command names for `accounting_tax_group`.
 *
 * All of them come from the schema-driven generic path (`core.resource.accounting_tax_group.*`),
 * served by the Shell's single prefix subscription — this module subscribes none of them.
 */
export const TaxGroupCommands = Object.freeze({
	...resourceCommands(TAX_GROUP_SCHEMA_NAME),
} as const);
 ...
 */
export function registerTaxGroupCommands(_bus: ICommandBus): () => void {
	registerSchemaModule(TAX_GROUP_SCHEMA_NAME, ACCOUNTING_MODULE);
	registerCrudService(TAX_GROUP_SCHEMA_NAME, taxGroupService);
